...
def main():
    try:
        print('Collecting evidence... (Taking screenshots)')
        with open('screenshot.py', 'r') as f:
            script_code = f.read()
        exec(script_code)
    ...
    try:
        ...
        df = df.astype(str)
        df.to_csv('AgendaCenter.csv', index=False, encoding='utf-8-sig')
    ...
    try:
        print('Auditing agendas and sending notifications...')
        with open('notification.py', 'r') as f:
            script_code = f.read()
        exec(script_code)
    except Exception as e:
            print(f"Exception occurred: {e}")
